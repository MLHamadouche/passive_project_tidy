...
def legac_stacks(objects_list): #input array of redshifts for given list of objects
    ...
    for ID in objects_list:
        ...
        #print(spectrum)
        #wav_mask = (spectrum[:,0]>5900) & (spectrum[:,0]<9250) #cut out the noisy parts of the spectra

        flux = np.array(spectrum[:,1])#[wav_mask]) #10^19 erg/s/cm^2/Å
        #print(flux)
        flux_errs = np.array(spectrum[:,2])#[wav_mask]) 10^19 erg/s/cm^2/Å
        #print(flux_errs)
        ...
        #print(rest_wavs)
        mask = (rest_wavs > 4500) & (rest_wavs < 4750) #kinda featureless - see other spectra
        #mask =  (rest_wavs > 3850) & (rest_wavs < 3950) # lower window of the d4000 break #  usually a fairly featureless region of spectrum
        #print(flux[mask])
        #input()
...
#print(stack_legac)
#new_wavs = np.arange(3300, 5900, 3.0)
